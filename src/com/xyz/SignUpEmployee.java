...
public class SignUpEmployee {
    ...
    public SignUpEmployee(Stage window, double height, double weight,Connection con,Session session) {
        this.height = height;
        this.weight = weight;
        this.window = window;
        this.con=con;
        ...
        GridPane.setConstraints(signUp,0,4,2,1, HPos.RIGHT, VPos.CENTER);
        signUp.setOnMouseClicked(event -> {
            ...
            if(_first.isEmpty() && _last.isEmpty() &&
                ...
                try {
                    createUser(_email,_first,_last,_uName,_password,_address,1);
                    session.uid=_email;
                    session.isImp=1;
                } catch (SQLException e) {
                    Statement statement = null;
                }
                if(session.uid!=null)window.setScene(new EmployeeHome(window, height, weight,con,session).getScene());
            }
        });
        ...
        center.getChildren().addAll(firstName,lastName,userName,email,password,address,signUp);
